...
export class Router extends ExtensibleHandler {
  ...
  doFork(forkedInstance, forkTable) {
    forkedInstance._routeLists = this._routeLists.map(
      routeList => routeList.doFork(forkTable))

    forkedInstance._defaultRouteList = 
      this._defaultRouteList.doFork(forkTable)

    forkedInstance._defaultHandler = 
      this._defaultHandler.doFork(forkTable)
