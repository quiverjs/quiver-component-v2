...
export class RouteList extends Component {
  ...
  privatize(privateCopy, bundle) {
    privateCopy._routes = this._routes.map(
      route => route.makePrivate(bundle))
...
export class Router extends ExtensibleHandler {
  ...
  privatize(privateCopy, bundle) {
    privateCopy._routeLists = this._routeLists.map(
      routeList => routeList.makePrivate(bundle))

    privateCopy._defaultRouteList = 
      this._defaultRouteList.makePrivate(bundle)

    privateCopy._defaultHandler = 
      this._defaultHandler.makePrivate(bundle)
